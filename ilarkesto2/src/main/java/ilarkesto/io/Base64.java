 ...
 * <li>v2.2.1 - Fixed bug using URL_SAFE and ORDERED encodings. Fixed bug when using very small files (~< 40 bytes).</li> <li>v2.2 - Added some helper methods
 * for encoding/decoding directly from one file to the next. Also added a main() method to support command line encoding/decoding from one file to the next.
 ...
 * <tt>decode( String s, boolean gzipCompressed )</tt>. Added the ability to "suspend" encoding in the Output Stream so you can turn on and off the encoding if
 * you need to embed base64 data in an otherwise "normal" stream (like an XML file).</li>
 ...
 * <li>v1.3.5 - Added flag to turn on and off line breaks. Fixed bug in input stream where last buffer being read, if not completely full, was not
 * returned.</li>
...
public class Base64 {
    ...
    private final static void usage(String msg) {
        err.println(msg);
        err.println("Usage: java Base64 -e|-d inputfile outputfile");
    } // end usage
     ...
     * bytes in your array is given by
     * <var>numSigBytes</var>. The array <var>threeBytes</var> needs only be as big as <var>numSigBytes</var>. Code can reuse a byte array by passing a
     ...
     * <var>destination</var>. The source and destination arrays can be manipulated anywhere along their length by specifying <var>srcOffset</var> and
     * <var>destOffset</var>. This method does not check to make sure your arrays are large enough to accomodate <var>srcOffset</var> + 3 for the
     * <var>source</var>
     * array or <var>destOffset</var> + 4 for the <var>destination</var> array. The actual number of significant bytes in your array is given by
     * <var>numSigBytes</var>.
     * </p>
     * <p>
     ...
     * <var>destination</var>. The source and destination arrays can be manipulated anywhere along their length by specifying <var>srcOffset</var> and
     * <var>destOffset</var>. This method does not check to make sure your arrays are large enough to accommodate <var>srcOffset</var> + 4 for the
     * <var>source</var>
     * array or <var>destOffset</var> + 3 for the <var>destination</var> array. This method returns the actual number of bytes that were converted from the
     * Base64 encoding.
     * <p>
    ...
    private static int decode4to3(byte[] source, int srcOffset, byte[] destination, int destOffset, int options) {
        ...
        if (source[srcOffset + 2] == EQUALS_SIGN) {
            ...
            try {
                ...
                out.println("" + source[srcOffset] + ": " + (DECODABET[source[srcOffset]]));
                out.println("" + source[srcOffset + 1] + ": " + (DECODABET[source[srcOffset + 1]]));
                out.println("" + source[srcOffset + 2] + ": " + (DECODABET[source[srcOffset + 2]]));
                out.println("" + source[srcOffset + 3] + ": " + (DECODABET[source[srcOffset + 3]]));
                return -1;
            } // end catch
        }
    } // end decodeToBytes
    ...
    public static byte[] decode(byte[] source, int off, int len, int options) {
        ...
        for (i = off; i < off + len; i++) {
            ...
            if (sbiDecode >= WHITE_SPACE_ENC) // White space, Equals sign or better
            {
                ...
                err.println("Bad Base64 input character at " + i + ": " + source[i] + "(decimal)");
                return null;
            } // end else:
    ...
    public static boolean encodeToFile(byte[] dataToEncode, String filename) {
        ...
        try {
            bos = new Base64.OutputStream(new java.io.FileOutputStream(filename), ENCODE);
    ...
    public static boolean decodeToFile(String dataToDecode, String filename) {
        ...
        Base64.OutputStream bos = null;
        try {
            bos = new Base64.OutputStream(new java.io.FileOutputStream(filename), DECODE);
    ...
    public static byte[] decodeFromFile(String filename) {
        ...
        try {
            ...
            if (file.length() > MAX_VALUE) {
                err.println("File is too big for this convenience method (" + file.length() + " bytes).");
                return null;
            } // end if: file too big for int index
            buffer = new byte[(int) file.length()];

            // Open a stream
            bis = new Base64.InputStream(new java.io.BufferedInputStream(new java.io.FileInputStream(file)), DECODE);
            ...
            err.println("Error decoding from file " + filename);
        } // end catch: IOException
        finally {
            try {
                if (bis != null) {
    ...
    public static String encodeFromFile(String filename) {
        ...
        try {
            // Set up some useful variables
            java.io.File file = new java.io.File(filename);
            byte[] buffer = new byte[max((int) (file.length() * 1.4), 40)]; // Need max() for math on
            // small files (v2.2.1)
            int length = 0;
            int numBytes = 0;
            ...
            err.println("Error encoding from file " + filename);
        } // end catch: IOException
        finally {
            try {
                bis.close();
    ...
    public static boolean encodeFileToFile(String infile, String outfile) {
        ...
        try {
            in = new Base64.InputStream(new java.io.BufferedInputStream(new java.io.FileInputStream(infile)), ENCODE);
    ...
    public static boolean decodeFileToFile(String infile, String outfile) {
        ...
        java.io.OutputStream out = null;
        try {
            in = new Base64.InputStream(new java.io.BufferedInputStream(new java.io.FileInputStream(infile)), DECODE);
            out = new java.io.BufferedOutputStream(new java.io.FileOutputStream(outfile));
            byte[] buffer = new byte[65536]; // 64K
            int read = -1;
            while ((read = in.read(buffer)) >= 0) {
