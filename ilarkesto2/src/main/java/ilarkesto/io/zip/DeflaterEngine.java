...
import static ilarkesto.io.zip.Deflater.FILTERED;
import static ilarkesto.io.zip.Deflater.HUFFMAN_ONLY;
...
class DeflaterEngine implements DeflaterConstants {
    ...
    /**
     * Hashtable, hashing three characters to an index for window, so that window[index]..window[index+2] have this hash code. Note that the array should really
     * be unsigned short, so you need to and the values with 0xffff.
     */
    private short[] head;

    /**
     * prev[index & WMASK] points to the previous index that has the same hash code as the string starting at index. This way entries with the same hash code
     * are in a linked list. Note that the array should really be unsigned short, so you need to and the values with 0xffff.
     */
    private short[] prev;
    ...
    /**
     * lookahead is the number of characters starting at strstart in window that are valid. So window[strstart] until window[strstart+lookahead-1] are valid
     * characters.
     */
    ...
    public void setLevel(int lvl) {
        ...
        if (COMPR_FUNC[lvl] != comprFunc) {
            if (DEBUGGING) {
                err.println("Change from " + comprFunc + " to "
                        + COMPR_FUNC[lvl]);
            }
            switch (comprFunc) {
                case DEFLATE_STORED:
    ...
    private void updateHash() {
        if (DEBUGGING) {
            err.println("updateHash: " + strstart);
        }
    ...
    private boolean findLongestMatch(int curMatch) {
        ...
        int best_len = max(matchLen, MIN_MATCH - 1);

        int limit = max(strstart - MAX_DIST, 0);
...
//  	  System.err.println("Found match: "+curMatch+"-"+(scan-strstart));
        ...
        matchLen = min(best_len, lookahead);
    ...
    void setDictionary(byte[] buffer, int offset, int length) {
        ...
        arraycopy(buffer, offset, window, strstart, length);
    ...
    private boolean deflateStored(boolean flush, boolean finish) {
        ...
        if ((storedLen >= MAX_BLOCK_SIZE)
            ...
            if (DEBUGGING) {
                err.println("storedBlock[" + storedLen + "," + lastBlock + "]");
            }
    ...
    public boolean deflate(boolean flush, boolean finish) {
        ...
        do {
            ...
            if (DEBUGGING) {
                err.println("window: [" + blockStart + "," + strstart + ","
