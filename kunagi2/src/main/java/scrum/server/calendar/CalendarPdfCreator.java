...
public class CalendarPdfCreator extends APdfCreator {
	...
	protected void build(APdfContainerElement pdf) {
		...
		for (SimpleEvent evt : events) {
			if (evt.getDate().isBefore(from)) {
                            continue;
            }
			if (evt.getDate().isAfter(to)) {
                            continue;
            }
			pdf.nl();
			...
			date.append(evt.getDate());
                        if (evt.isTimeSet()) {
                            date.append(" ").append(evt.getTime());
            }
                        pdf.paragraph().text(date + " " + evt.getLabel(), headerFonts[2]);
                        if (evt.isNoteSet()) {
                wiki(pdf, evt.getNote());
            }
			pdf.nl();
                        FieldList fields = pdf.fieldList().setLabelFontStyle(fieldLabelFont);
                        if (evt.isDurationSet()) {
                            fields.field("Duration").text(evt.getDuration() + " min.");
            }
                        if (evt.isLocationSet()) {
                            fields.field("Location").text(evt.getLocation());
            }
			if (evt.isAgendaSet()) {
                wiki(fields.field("Agenda"), evt.getAgenda());
            }
		}
	}
