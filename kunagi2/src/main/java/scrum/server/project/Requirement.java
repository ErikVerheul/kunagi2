...
public class Requirement extends GRequirement implements Numbered, ReferenceSupport, LabelSupport {
     ...
     */
		...
		for (Change change : changes) {
			...
			if (scrum.client.journal.Change.REQ_COMPLETED_IN_SPRINT.equals(key)
                                || scrum.client.journal.Change.REQ_REJECTED_IN_SPRINT.equals(key)) {
    ...
    public Set<Change> getSprintSwitchChanges() {
		...
		while (iterator.hasNext()) {
			Change change = iterator.next();
                        if (change.isKey(scrum.client.journal.Change.REQ_COMPLETED_IN_SPRINT)) {
                continue;
            }
                        if (change.isKey(scrum.client.journal.Change.REQ_REJECTED_IN_SPRINT)) {
                continue;
            }
			iterator.remove();
		}
    ...
    public Set<Task> getTasksInSprint(Sprint sprint) {
		...
		while (iterator.hasNext()) {
			...
			if (task.isClosedInPastSprintSet() || !task.isSprint(sprint)) {
                iterator.remove();
            }
		}
		return tasks;
    ...
    public String getWorkDescriptionAsString() {
		if (!isInCurrentSprint()) {
            return Str.appendIfNotBlank(getEstimatedWorkAsString(), "SP");
        }
    ...
    public void initializeEstimationVotes() {
        for (User user : getProject().getTeamMembers()) {
			...
			if (vote == null) {
                vote = requirementEstimationVoteDao.postVote(this, user);
            }
	...
	public void updateNumber() {
		if (getNumber() == 0) {
                    setNumber(getProject().generateRequirementNumber());
                }
        }
	...
	public void ensureIntegrity() {
		...
		// delete estimation votes when closed or in sprint
                if (isClosed() || isSprintSet()) {
                    clearEstimationVotes();
        }
	}
