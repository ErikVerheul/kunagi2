...
public class Deflater {
        ...
        /**
         * This strategy will not look for string repetitions at all. It only encodes with Huffman trees (which means,
         * that more common characters get a smaller encoding.
         */
        public static final int HUFFMAN_ONLY = 2;

        /**
         * The compression method. This is the only method supported so far. There is no need to use this constant at
         * all.
         */
         ...
         *
         * @param lvl the compression level, a value between NO_COMPRESSION and BEST_COMPRESSION, or
         * DEFAULT_COMPRESSION.
         ...
         * @param nowrap true, iff we should suppress the deflate header at the beginning and the adler checksum at the
         * end of the output. This is useful for the GZIP format.
         * @exception IllegalArgumentException if lvl is out of range.
         */
        ...
        /**
         * Resets the deflater. The deflater acts afterwards as if it was just created with the same compression level
         * and strategy as it had before.
         */
        public void reset() {
        ...
        /**
         * Frees all objects allocated by the compressor. There's no reason to call this, since you can just rely on
         * garbage collection. Exists only for compatibility against Sun's JDK, where the compressor allocates native
        ...
        /**
         * Flushes the current input block. Further calls to deflate() will produce enough output to inflate everything
         * in the current input block. This is not part of Sun's JDK so I have made it package private. It is used by
         ...
         * input is needed. If you call setInput when needsInput() returns false, the previous input that is still
         * pending will be thrown away. The given byte array should not be changed, before needsInput() returns true
         * again. This call is equivalent to <code>setInput(input, 0, input.length)</code>.
         *
         ...
         * @exception IllegalStateException if the buffer was finished() or ended() or if previous input is still
         * pending.
         */
        ...
        /**
         * Sets the compression level. There is no guarantee of the exact position of the change, but if you call this
         * when needsInput is true the change of compression level will occur somewhere near before the end of the so
         * far given input.
         *
        ...
        /**
         * Deflates the current input block to the given array. It returns the number of bytes compressed, or 0 if
         * either needsInput() or finished() returns true or length is zero.
         *
         ...
         * @exception IllegalStateException if setInput () or deflate () were already called or another dictionary was
         * already set.
         */
         ...
         * containing strings that are likely to occur in the data which should be compressed. The dictionary is not
         * stored in the compressed output, only a checksum. To decompress the output you need to supply the same
         * dictionary again.
         *
         * @param dict the dictionary.
